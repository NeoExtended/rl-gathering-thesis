...
def conv(input_tensor, scope, *, n_filters, filter_size, stride,
         pad='VALID', initializer=ortho_init(1.0), data_format='NHWC', one_dim_bias=False):
    """
    ...
    with tf.variable_scope(scope):
        weight = tf.get_variable("w", wshape, initializer=initializer)
...
def linear(input_tensor, scope, n_hidden, *, initializer=ortho_init(1.0), init_bias=0.0):
    """
