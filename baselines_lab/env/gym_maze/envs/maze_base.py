...
class MazeBase(gym.Env):
    ...
    def _update_locations(self, new_locations):
        ...
        valid_locations = self.freespace.ravel()[(new_locations[:, 1] + new_locations[:, 0] * self.freespace.shape[1])] # Border does not need to be checked as long as all maps have borders.
        self.particle_locations = np.where(valid_locations[:, np.newaxis], new_locations, self.particle_locations)
