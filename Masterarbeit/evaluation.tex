...
\begin{table}[htp]
    \begin{center}
        \begin{tabular}{crrrrrr}
            ...
            \multicolumn{1}{c}{Algorithm} & \multicolumn{1}{c}{CV} & \multicolumn{1}{c}{Delta} & \multicolumn{1}{c}{CV} & \multicolumn{1}{c}{Delta} & \multicolumn{1}{c}{CV} & \multicolumn{1}{c}{Delta} \\
            \midrule
            PPO & 7.11\% & 10.81 & 4.74\% & 10.89 & 5.53\% & 44.58\\
            \bottomrule
        \end{tabular}
    \end{center}
    \caption[Result Variation Using Different RL Algorithms]{Result variation in terms of averaged final episode length during evaluation in absolute numbers (difference between maximum and minimum) and as coefficient of variation (CV) ($\sigma / \mu$) on the three test instances. The results in this table are calculated from the results of the reward experiments excluding experiments using DEL, curiosity and experiments without normalization. The results are comparable to the results calculated with lower sample size in Table \ref{tab:Algorithm/ResultVariation}.} \label{tab:Algorithm/ResultVariation2}
\end{table}

Because the data we use is very limited due to each experiment being only repeated three times, the error in Table \ref{tab:Algorithm/ResultVariation} can be quite large. We therefore also analyze the variation in past experiments. Using data from the reward experiments with continuous reward and excluding experiments with DEL, curiosity reward or without normalization. Using this data, we can compute the result variation as an average over 15 trials for Corridor and Vessel and 9 trials for Brain. The results are shown in Table \ref{tab:Algorithm/ResultVariation2}. We can see, that even with a larger sample size, we get comparable results. In praxis this means, that independent of the training randomness, we will still get a usable result at some point. Results of single trials are comparable within a certain error bound, but evaluation of close results still requires multiple trials.
